...
func (r *ApplicationPhaseCommentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if currentRevision != appHealth.Status.LastSyncOperation.Revision ||
		phase != appHealth.Status.LastSyncOperation.Phase {
		...
		appHealth.Status.LastSyncOperation = argocdcommenterv1.LastSyncOperation{
